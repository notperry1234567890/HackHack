...
public class InventoryUtil implements Util {
  ...
  private static Map<Integer, ItemStack> getInventorySlots(int currentI, int last) {
    ...
    while (current <= last) {
      fullInventorySlots.put(Integer.valueOf(current), (ItemStack)mc.player.inventoryContainer.getInventory().get(current));
      current++;
    } 
    ...
    switch (mode) {
      ...
      case SILENT:
        if (!back && !switchedItem) {
          ...
          switchedItemSwitched[0] = false;
          GameSenseMod.inventoryManager.recoverSilent(lastHotbarSlot);
        } 
        switchedItemSwitched[1] = true;
        break;
      case NONE:
