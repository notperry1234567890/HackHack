...
public class WorldUtils {
    ...
    for (int var5 = 0; var5 < var4; var5++) {
      ...
      if ((Minecraft.getMinecraft()).world.getBlockState(neighbor).getBlock().canCollideCheck((Minecraft.getMinecraft()).world.getBlockState(neighbor), false)) {
        ...
        if (eyesPos.squareDistanceTo(hitVec) <= 18.0625D) {
          ...
          float[] rotations = { (Minecraft.getMinecraft()).player.rotationYaw + MathHelper.wrapDegrees(yaw - (Minecraft.getMinecraft()).player.rotationYaw), (Minecraft.getMinecraft()).player.rotationPitch + MathHelper.wrapDegrees(pitch - (Minecraft.getMinecraft()).player.rotationPitch) };
  ...
  public static BlockPos getRelativeBlockPos(EntityPlayer player, int ChangeX, int ChangeY, int ChangeZ) {
    BlockPos pos;
    int[] playerCoords = { (int)player.posX, (int)player.posY, (int)player.posZ };
