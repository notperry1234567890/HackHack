...
public class OffhandGap extends Module {
  ...
  public void onDisable() {
    if (OffhandCrystal.mc.currentScreen instanceof net.minecraft.client.gui.inventory.GuiContainer)
      return; 
  ...
  public void onUpdate() {
    ...
    if (shouldTotem() && OffhandCrystal.mc.player.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING) {
      this.totems++;
    } else if (!shouldTotem() && OffhandCrystal.mc.player.getHeldItemOffhand().getItem() == this.item) {
      ...
      if (OffhandCrystal.mc.player.inventory.getItemStack().isEmpty()) {
        if (!shouldTotem() && OffhandCrystal.mc.player.getHeldItemOffhand().getItem() == this.item)
          return; 
        if (shouldTotem() && OffhandCrystal.mc.player.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING)
          return; 
        if (!shouldTotem()) {
  ...
  private boolean shouldTotem() {
    boolean hp = (OffhandCrystal.mc.player.getHealth() + OffhandCrystal.mc.player.getAbsorptionAmount() <= this.health.getValue());
    boolean endcrystal = !isCrystalsAABBEmpty();
    return hp;
  }
  
  private boolean isEmpty(BlockPos pos) {
    List<Entity> crystalsInAABB = (List<Entity>)OffhandCrystal.mc.world.getEntitiesWithinAABBExcludingEntity(null, new AxisAlignedBB(pos)).stream().filter(e -> e instanceof net.minecraft.entity.item.EntityEnderCrystal).collect(Collectors.toList());
    return crystalsInAABB.isEmpty();
  }
  
  private boolean isCrystalsAABBEmpty() {
    return (isEmpty(OffhandCrystal.mc.player.getPosition().add(1, 0, 0)) && isEmpty(OffhandCrystal.mc.player.getPosition().add(-1, 0, 0)) && isEmpty(OffhandCrystal.mc.player.getPosition().add(0, 0, 1)) && isEmpty(OffhandCrystal.mc.player.getPosition().add(0, 0, -1)) && isEmpty(OffhandCrystal.mc.player.getPosition()));
  }
}
