...
public abstract class InjectionPoint {
  ...
  private static void validateByValue(IMixinContext context, MethodNode method, AnnotationNode parent, InjectionPoint point, int by) {
    ...
    if (by <= allowed)
      return; 
    String message = String.format("@%s(%s) Shift.BY=%d on %s::%s exceeds the maximum allowed value %d.", new Object[] { Bytecode.getSimpleName(parent), point, 
          Integer.valueOf(by), context, method.name, Integer.valueOf(allowed) });
